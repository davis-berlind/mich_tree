tree_mat <- function(n_leaf, edges) {
  adj <- matrix(0, nrow = n_leaf, ncol = max(edges) - n_leaf)
  for (i in 1:n_leaf) {
    j <- i
    while (TRUE) {
      j <- edges[edges[,2] == j, 1]
...
tree_sim <- function(n, L, min_space = 10, C = 40, sd = 1, detect_window) {
  ...
  while (length(active_nodes) < L) {
    ...
    active_color[node] <- color
    leaf_color <- adj %*% active_color
    if (min(table(leaf_color)) >= min_space) {
      active_nodes <- c(active_nodes, node + n)
      color <- color + 1
      parent <- edges[edges[,2] == node + n, 1]
      sibs <- unlist(edges[edges[,1] == parent, 2])
      cand <- setdiff(cand, sibs - n)
    } else {
      active_color[node] <- 0
...
window_fn <- function(chp, adj, wp = 0.5) {
  unlist(
    lapply(chp - nrow(adj), function(x) {
      y = adj + adj[,x];
      which(colSums(y == 1) / colSums(y >= 1) < wp) + nrow(adj)
    })
  )
}

tree_hausdorff <- function(est_cp, true_cp, tree, n_leaf) {
